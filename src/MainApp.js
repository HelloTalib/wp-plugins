...
function MainApp() {
  ...
  const [sortOption, setSortOption] = useState(
    initialUi?.sortOption ?? "installation",
  );
  const [latestWpVersion, setLatestWpVersion] = useState("6.7");

  useEffect(() => {
    fetch(WP_VERSION_CHECK_URL)
      .then((r) => r.json())
      .then((data) => {
        const ver = data?.offers?.[0]?.current;
        if (ver) setLatestWpVersion(ver);
      })
      .catch(() => {});
  }, []);
  ...
  const getUpdatedMeta = useCallback(
    (dateString) => {
      ...
      if (!updatedDate)
        return { days: Infinity, color: "#ef4444", bg: "rgba(239,68,68,0.1)" };
      const now = new Date();
      const days = Math.max(
        0,
      ...
      if (days <= 14)
        return { days, color: "#059669", bg: "rgba(16,185,129,0.1)" };
      if (days <= 30)
        return { days, color: "#d97706", bg: "rgba(245,158,11,0.1)" };
      return { days, color: "#ef4444", bg: "rgba(239,68,68,0.1)" };
    },
    [parseWpDate],
  );

  const calculatePluginAge = useCallback(
    (dateString) => {
  ...
  const calculateLastUpdated = useCallback(
    (dateString) => {
      ...
      if (totalMonths < 12) {
        const days = totalDays % 30;
        return days > 0
          ? `${totalMonths}mo ${days}d ago`
          : `${totalMonths}mo ago`;
      }
      const months = totalMonths % 12;
      return months > 0
        ? `${totalYears}yr ${months}mo ago`
        : `${totalYears}yr ago`;
    },
    [parseWpDate],
  );
    ...
    async (authorName, pluginSearchName, tagSearchName, page = 1) => {
      ...
      try {
        ...
        if (authorName) params.push(`author=${encodeURIComponent(authorName)}`);
          ...
          params.push(`search=${encodeURIComponent(pluginSearchName)}`);
        else if (tagSearchName)
          params.push(`tag=${encodeURIComponent(tagSearchName)}`);
        ...
        setTotalPages(
          Math.max(1, Math.min(MAX_PAGE, Math.ceil(resultCount / perPage))),
        );
      } catch (fetchError) {
  ...
  const displayedPlugins = useMemo(() => {
    ...
    sorted.sort((a, b) => {
      ...
      if (sortOption === "updated") {
        return (
          (parseWpDate(b.last_updated)?.getTime() || 0) -
          (parseWpDate(a.last_updated)?.getTime() || 0)
        );
      }
      ...
      >
        ...
        >
          ...
          <Typography
            ...
            sx={{ fontWeight: 500 }}
          >
            Data from WordPress.org · Built with React & MUI · ©{" "}
